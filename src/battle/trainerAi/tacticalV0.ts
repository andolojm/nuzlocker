...
import { drainFraction, healFraction, isImmuneViaAbility, recoilFraction } from "./damageMath";
import { accuracyStageMultiplier, compareSpeed, statStageMultiplier } from "./statMath";
// Frozen copies of the two damage functions that have since changed. Importing the live ones would
// quietly hand v0 the crit expectation and the resolved base powers it is meant to predate.
import { computeRawDamageV0 as computeRawDamage, computeSpecDamageV0 as computeSpecDamage } from "./tacticalV0Damage";
import type { Combatant, FieldConditions, SpeedComparison, TrainerAiContext, TrainerAiImplementation } from "./types";

/*
 * FROZEN SNAPSHOT — do not develop this file.
 *
 * The "tactical" AI as it stood before the switching, residual-damage, crit, variable-base-power,
 * accuracy and scoring work landed. It is kept selectable so the two can be played against each
 * other and so there is something to fall back to if a change to the live AI turns out badly.
 * Improvements belong in tactical.ts; the only edits this file should ever take are the mechanical
 * ones needed to keep it compiling (see tacticalV0Damage.ts for how its damage math is pinned).
 *
...
export const tacticalV0TrainerAi: TrainerAiImplementation = {
  id: "tactical-v0",
  label: "Tactical v0",
  chooseMove,
};
