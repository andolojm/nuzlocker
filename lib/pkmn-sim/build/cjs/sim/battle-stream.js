...
class BattleStream extends lib_1.Streams.ObjectReadWriteStream {
    ...
    _writeLines(chunk) {
        for (const line of chunk.split('\n')) {
            if (line.startsWith('>')) {
                const [type, message] = lib_1.Utils.splitFirst(line.slice(1), ' ');
    ...
    _writeLine(type, message) {
        switch (type) {
            case 'start':
                ...
                break;
            case 'player':
                const [slot, optionsText] = lib_1.Utils.splitFirst(message, ' ');
    ...
    editbattle(target) {
        ...
        const getPokemon = (side, input) => {
            if (typeof side === 'string')
                side = getPlayer(side);
            input = toID(input);
            let out = null;
            if (/^[1-9]$/.test(input))
                out = side.pokemon[parseInt(input) - 1];
            else
                out = side.pokemon.find(p => p.baseSpecies.id === input || p.species.id === input);
            if (!out)
                throw new Error(`Pokemon "${side.name} ${input}" not found`);
            return out;
        };
        ...
        if (target.startsWith('user:')) {
            [user, target] = lib_1.Utils.splitFirst(target.slice(5), ',');
            target = target.trim();
        }
        battle.add('html', lib_1.Utils.html `<div class="message-warning">${user ? `[${user}] ` : ''}<strong>/editbattle</strong> ${target}</div>`);
        let cmd;
        ...
        switch (cmd) {
            ...
            case 'h': {
                ...
                if (p.isActive)
                    battle.add('-damage', p, p.getHealth);
                break;
            }
            case 'status':
            case 's': {
            ...
            case 'reseed': {
                battle.resetRNG(target);
                if (targets.length)
                    battle.add(`||Reseeded to ${targets.join(',')}`);
                break;
            }
            default:
...
function getPlayerStreams(stream) {
    ...
    (async () => {
        for await (const chunk of stream) {
            const [type, data] = lib_1.Utils.splitFirst(chunk, `\n`);
            switch (type) {
                case 'update':
                    ...
                    break;
                case 'sideupdate':
                    const [side, sideData] = lib_1.Utils.splitFirst(data, `\n`);
...
class BattlePlayer {
    ...
    receiveLine(line) {
        ...
        if (!line.startsWith('|'))
            return;
        const [cmd, rest] = lib_1.Utils.splitFirst(line.slice(1), '|');
        if (cmd === 'request')
