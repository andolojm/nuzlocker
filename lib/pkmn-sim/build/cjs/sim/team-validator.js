...
class PokemonSources {
    ...
    addGoIncompatibleMove(move) {
        if (!this.goIncompatibleMoves)
            this.goIncompatibleMoves = [];
        if (move)
            this.goIncompatibleMoves.push(move);
    ...
    validateStats(set, species, setSources, pokemonGoProblems) {
        ...
        for (const stat in set.evs) {
            if (set.evs[stat] < 0) {
                ...
                problems.push(`${name} has less than 0 ${statValue} in ${dex_1.Dex.stats.names[stat]}.`);
            }
        }
        if (dex.currentMod === 'gen7letsgo') { // AVs
            ...
            for (const stat in set.evs) {
                if (set.evs[stat] > 32) {
                    problems.push(`${name} has more than 32 Stat Points in ${dex_1.Dex.stats.names[stat]}.`);
                }
            }
        }
        else { // EVs
    ...
    checkCanLearn(move, originalSpecies, setSources = this.allSources(originalSpecies), set = {}) {
        ...
        for (const { species, learnset } of fullLearnset) {
            ...
            for (let learned of sources) {
                ...
                const canUseAbilityPatch = dex.gen >= 8 && format.mod !== 'gen8dlc1';
                if (learnedGen < 7 && setSources.isHidden && !canUseAbilityPatch && onlyLegalAbilities &&
