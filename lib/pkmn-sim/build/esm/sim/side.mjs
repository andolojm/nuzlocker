    ...
    commitChoices() {
        if (this.battle.gen === 1) {
            for (const choice of this.choice.actions) {
                if (choice.choice !== 'move' || !choice.pokemon)
                    continue;
    ...
    choose(input) {
        ...
        for (const choiceString of choiceStrings) {
            ...
            switch (choiceType) {
                ...
                case 'decline':
                    if (data)
                        return this.emitChoiceError(`Unrecognized data after "decline": ${data}`);
                    if (!this.chooseDecline())
                        return false;
                    break;
                case 'auto':
                case 'default':
