...
export class Pokemon {
    constructor(set, side) {
        ...
        if (this.battle.format.pokemon)
            Object.assign(this, this.battle.format.pokemon);
    ...
    */
    ...
    getMoveSlot(slotIndex) {
        // used by Gen 1
        if (slotIndex < 0 || slotIndex >= this.moveSlots.length)
            return null;
    ...
    getSemiLockedMove(restrictData) {
        if (restrictData && this.maybeLocked)
            return null;
    ...
    takeItem(source) {
        if (!source)
            source = this;
        if (this.battle.gen <= 4 && (this.itemKnockedOff || source.itemKnockedOff))
            return false;
        if (!this.item)
            return;
        const item = this.getItem();
    ...
    runEffectiveness(move) {
        ...
        if (this.species.name === 'Terapagos-Terastal' && this.hasAbility('Tera Shell') &&
            ...
            if (move.hit === 1)
                delete this.abilityState.resisted; // reset for first hit
