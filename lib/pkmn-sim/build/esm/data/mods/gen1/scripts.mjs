        ...
        runMove(moveOrMoveName, pokemon, targetLoc, options) {
            ...
            if (move.id === 'cannotmove') {
                if (pokemon.status === 'slp') {
                    this.battle.hint("In Gen 1, if a Pokémon spends a turn partially trapped and switches to a Pokémon that is asleep, " +
                        "the sleep counter will not decrease until you select a move with a different Pokémon.");
                }
                else if (pokemon.getLockedMove()) {
                    this.battle.hint("In Gen 1, when Haze cures the sleep/freeze status of a Pokémon during a multi-turn move, " +
                        "that Pokémon will become soft-locked.");
                }
                else if (pokemon.getSemiLockedMove()) {
                    this.battle.hint("In Gen 1, when Haze cures the sleep/freeze status of a Pokémon during Bide, " +
            ...
            if (move.id !== 'struggle') {
                ...
                if ((!lockedMove && !TWO_TURN_MOVES.includes(move.id)) || pokemon.volatiles['twoturnmove']) {
                    ...
                    if (moveSlot)
                        pokemon.deductPP(moveSlot.id, null, target);
                }
                if (!lockedMove && move.id !== pokemon.getMoveSlot(pokemon.side.lastSelectedMoveSlot)?.id) {
                    ...
                    this.battle.hint("In Gen 1, a Pokémon might default to using a move that doesn't match the move of the slot it last selected.");
                    abortMove();
                    return;
                }
            }
        ...
        useMove(moveOrMoveName, pokemon, options) {
            ...
            if (sourceEffect?.id === 'metronome' || sourceEffect?.id === 'mirrormove') {
                if (TWO_TURN_MOVES.includes(move.id)) {
                    const moveSlot = pokemon.getMoveSlot(pokemon.side.lastSelectedMoveSlot);
                    if (moveSlot)
                        pokemon.deductPP(moveSlot.id, -1, target);
                }
