/**
 * Regenerates src/vendor/pokemon-data/abilities.json. Run from the repo root: `node
 * scripts/generate-abilities.cjs` (needs @pkmn/sim installed).
 *
 * Source: https://pokemondb.net/ability (scraped 2026-09-09) — name + one-line description. The
 * `hidden` flag is a local curation (see README "abilities.json"): it excludes an ability from the
 * random-assignment pool. It's set for the HIDDEN list below plus any ability @pkmn/sim has no
 * generic entry for. If you re-scrape, update ROWS, keep the alphabetical order (ids are 1-based
 * positions), and re-check HIDDEN against the current sim data.
 */
const fs = require("fs");
const path = require("path");
const { toID, Dex } = require("@pkmn/sim");

// [name, description] rows from pokemondb.net/ability. Quick Draw's description was missing on the
// page and is filled in from the in-game text.
const ROWS = [
  ...
  ["Dragonize", "Turns Normal-type moves into Dragon-type moves."],
...
// Curated exclusions from the random-ability pool: abilities that are a complete no-op on an
// arbitrary Pokemon in this game's 1v1 (singles) battles. See README "abilities.json".
const HIDDEN = new Set([
  // Form/species-locked — the sim hard-gates the effect to one species, so it does nothing else.
  "Battle Bond", "Disguise", "Flower Gift", "Forecast", "Gulp Missile", "Hunger Switch",
  "Ice Face", "Multitype", "Power Construct", "RKS System", "Schooling", "Shields Down",
  "Stance Change", "Tera Shell", "Tera Shift", "Teraform Zero", "Zen Mode", "Zero to Hero",
  // Ally-targeting / doubles-only — no ally ever exists in a singles battle.
  "Battery", "Commander", "Costar", "Curious Medicine", "Friend Guard", "Healer", "Hospitality",
  "Power of Alchemy", "Power Spot", "Receiver", "Symbiosis", "Telepathy",
  // Purely out-of-battle in the games; the sim implements no battle effect at all.
  "Ball Fetch", "Honey Gather", "Run Away",
  // No @pkmn/sim entry under this name (species-form-specific in the sim: As One -> Calyrex,
  // Embody Aspect -> Ogerpon).
  "As One", "Embody Aspect",
]);

const notInSim = [];
const data = ROWS.map(([name, description], i) => {
  const exists = Dex.abilities.get(toID(name)).exists;
  if (!exists) notInSim.push(name);
  return { id: i + 1, name, description, hidden: !exists || HIDDEN.has(name) };
});

const unknownHidden = [...HIDDEN].filter((n) => !ROWS.some(([name]) => name === n));
if (unknownHidden.length) throw new Error("HIDDEN names not in ROWS: " + unknownHidden.join(", "));

const repoOut = path.resolve("src/vendor/pokemon-data/abilities.json");
fs.writeFileSync(repoOut, JSON.stringify(data, null, 2) + "\n");

console.log("wrote", repoOut, "count", data.length);
console.log("not in sim (marked hidden):", notInSim.length ? notInSim.join(", ") : "none");
